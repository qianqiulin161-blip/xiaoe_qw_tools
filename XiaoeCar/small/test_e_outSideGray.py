...
from XiaoeCar.same.Same import build_plan, get_upload_appId_system_statu, up_appId
...
# 准现网验证报告生成
def _generate_alert_message(plan_name, total_dan, down_count, final, ui_final):
    if r.get(smallConfig.department_config[25]):
        res_plan_content = in_plan(r.get(smallConfig.department_config[4])).json()['data']['list']
        now_names = []
        for i in res_plan_content:
            now_names.append(i['iteration_id'])
        last_names = ast.literal_eval(r.get(smallConfig.department_config[25]))
...
@retry(stop=stop_after_attempt(MAX_ATTEMPTS), wait=wait_fixed(RETRY_INTERVAL),
       reraise=True)
def _process_plan(plan_key, redis_key, plan_type_str):
    plan_id = r.get(plan_key)
    taskId = getTaskId(plan_id)
    
    if not plan_id:
        Logger.debug(f"{plan_type_str}没有小车计划")
        return
    ...
    if not (set(content) == set(target_list) or len(result["data"]["list"]) != valid_count):
        ...
        alert_data = _generate_alert_message(plan_detail.json()['data']['plan_name'], int(r.get(smallConfig.department_config[15])),
                                             len(response.json()['data']['list']), final_content, ui_final_content)
        robot_smallCar(alert_data, smallConfig.robotWebHook)
        r.set(redis_key, str(target_list))
        
    # 国内准现网自动添加外灰名单
    add_outside_gray(plan_id, valid_count, result, "国内-准现网", smallConfig.AddAppIdList, 131, "xiaoetong-pro", smallConfig.department_config[18])
        
    # 国外准现网自动添加外灰名单
    if taskId.get('海外-准现网'):
        if build_plan(plan_id, taskId['海外-准现网']):
            add_outside_gray(plan_id, valid_count, result, "海外-准现网", smallConfig.OverseasAddAppIdList, 129, "elink-pro", smallConfig.department_config[27])
    Logger.debug(f"{plan_type_str}: {target_list}, {valid_count}")



    # 自动添加外灰名单
def add_outside_gray(plan_id, valid_count, result, environment, appId_list, label_id, culster_id, redis_index):
    now = datetime.now()
...
def _control_add_appid(plan_id):
    """控制自动添加外灰名单"""
    self_content = {
                "msgtype": "markdown",
                "markdown": {
    ...
    self_result = r.get(smallConfig.PartPlanName)
    if self_result:
        self_result = ast.literal_eval(self_result)
        Logger.debug(f"今日接口自动化记录结果 {self_result}")
        if  any(all(j == "PASS" for j in i) for i in self_result) and r.get(smallConfig.department_config[24]+str(plan_id)) in ['1', '2']:
            return True
...
def test_outSideGray():
    # 提醒测试
    Remind_TesterToTest()

    _process_plan(smallConfig.department_config[4], smallConfig.department_config[8], "今日")


def getAllTester(planId):
    """构造提醒测试小车单的消息"""
    testers = [i['creator'] for i in in_plan(r.get(smallConfig.department_config[4])).json()['data']['list']]
    ...
    return {
        ...
        "markdown": {
            "content": f"{GetUserId(list(set(testers)), [])[0]} 可以开始验证小车啦！\n\n"
                       ...
                       f"已加入【国内-准现网】的名单有： {'  '.join(appId for appId in app_list)}"
        }
    }


def Remind_TesterToTest():
    """提醒主控制逻辑"""
    if r.get(smallConfig.department_config[4]):

        if getTaskId(r.get(smallConfig.department_config[4])).get('国内-准现网'):
            send_msg = getAllTester(r.get(smallConfig.department_config[4]))
            success_count = HadAppId()
            taskId = getTaskId(r.get(smallConfig.department_config[4]))
            Check_system = get_upload_appId_system_statu(r.get(smallConfig.department_config[4]), taskId['国内-准现网'],
                                                   '【国内-准现网】有系统发布失败 请检查 !!!',
                                                   at_tester, at_driver, smallConfig.robotWebHook, '小车计划')
            Logger.debug(f"获取到的taskId {taskId}, success_count {success_count}, Check_system {Check_system}, {r.get(smallConfig.department_config[11])} ")
            if (r.get(smallConfig.department_config[11]) == "0.5" or r.get(smallConfig.department_config[11]) == "0") and success_count >= 1 and Check_system is True:
                robot_smallCar(send_msg, smallConfig.robotWebHook)
                r.set(smallConfig.department_config[11], "1")
                Logger.debug("已提醒测试人员测试")
            elif r.get(smallConfig.department_config[11]) == "1":
                run_automation(r.get(smallConfig.department_config[4]))