...
def get_week_person(Testers1, ReviewPerson1, Drivers1, DriverTag1):
    ...
    if r.get(smallConfig.department_config[20]):
        ...
        if cache[0] == date_key:
            ...
            if (len(Testers1) == cache[1] + 1) and (len(ReviewPerson1) == cache[2] + 1):
                r.set(smallConfig.department_config[20], str([date_key, 0, 0]))
                tester = Testers1[0]
                personOfReview = ReviewPerson1[0]
            elif len(Testers1) == cache[1] + 1:
                r.set(smallConfig.department_config[20], str([date_key, 0, cache[2] + 1]))
                tester = Testers1[0]
                personOfReview = ReviewPerson1[cache[2] + 1]
            elif len(ReviewPerson1) == cache[2] + 1:
                r.set(smallConfig.department_config[20], str([date_key, cache[1] + 1, 0]))
                tester = Testers1[cache[1] + 1]
                personOfReview = ReviewPerson1[0]
            else:
                r.set(smallConfig.department_config[20], str([date_key, cache[1] + 1, cache[2] + 1]))
                tester = Testers1[cache[1] + 1]
    ...
    if r.get(smallConfig.department_config[21]):
        driver_cache = ast.literal_eval(r.get(smallConfig.department_config[21]))
        if DriverTag1 == 1:
            if driver_cache[0] == today:
                driver = Drivers1[driver_cache[1]]
            elif driver_cache[0] != today:
                if len(Drivers1) == driver_cache[1] + 1:
                    r.set(smallConfig.department_config[21], str([today, 0]))
                    driver = Drivers1[0]
                else:
                    r.set(smallConfig.department_config[21], str([today, driver_cache[1] + 1]))
...
def at_person():
    ...
    if baseConfig.planType == '0':
        ...
        if isHoliday or weekDay in smallConfig.relaxWorkDay:
            at_list = ast.literal_eval(r.get(smallConfig.department_config[22]))
            at = at_list[0]
            at_tester = at_list[1]
            at_driver = at_list[2]
            at_tester_Id = None
            at_driver_Id = None
        elif isWork or smallConfig.PartPlanName == '零售产品中心':
